...
public class MagicGathererPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	public static final String RECENTS_DESC = "Recents (last 2 years)";
	...
	protected void createFieldEditors() {
		...
		/*
		 * !!! RD Deprecated, we are always downloading every variations now
		 ...
		 * BooleanFieldEditor(PreferenceConstants.GATHERER_UPDATE_SPECIAL,
		 * "Load special items", getFieldEditorParent()));
		 */
		/*
		 * !!! RD Not supported for now, using default "list" provided by Scryfall for
		 * now addField(new
		 * SpecialComboFieldEditor(PreferenceConstants.GATHERER_UPDATE_LANGUAGE,
		 * "Also load localized version in:", createLanguagesArray(),
		 * getFieldEditorParent(), SWT.DROP_DOWN)); Collection<String> names =
		 * Editions.getInstance().getNames(); String proposals[] = new
		 * String[names.size()]; int i = 0; for (String type : names) { proposals[i++] =
		 * type; }
		 * 
