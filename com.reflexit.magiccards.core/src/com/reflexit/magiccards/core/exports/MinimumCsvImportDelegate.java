...
public class MinimumCsvImportDelegate extends CsvImportDelegate {
	LinkedHashMap<String, ICardField> fieldMap = new LinkedHashMap<>();

	public MinimumCsvImportDelegate() {
		...
		fieldMap.put("GATHERERID", MagicCardField.GATHERERID);
		fieldMap.put("ID", MagicCardField.ID);
		fieldMap.put("OWNERSHIP", MagicCardField.OWNERSHIP);
		fieldMap.put("EDITION_ABBR", MagicCardField.EDITION_ABBR);
	}

	public static String HEADER = "NAME,SET,COUNT,SPECIAL,COMMENT,LANG,COLLNUM,GATHERERID,ID,OWNERSHIP,EDITION_ABBR";

	/*-
	 Count,Name,Language,Card Number, GatheredId, ManaDeskId
	 Platinum Angel,Magic 2010,3,,,English,218,,
	 Reya Dawnbringer,Duel Decks: Divine vs. Demonic,,,English,13,,
	 Ashes of the Abhorrent,Ixalan,2,,,English,,,true
