...
public class ParseGathererSearchChecklist extends AbstractParseGathererSearch {
	...
	public boolean processFromReader(BufferedReader st, GatherHelper.ILoadCardHander handler) throws IOException {
		...
		/*
		 * String line = ""; boolean cards = false; boolean lastpage = false; while
		 * ((line = st.readLine()) != null) { if (countPattern.matcher(line).find()) {
		 * Matcher matcher = countPattern.matcher(line); try { matcher.find(); int c =
		 * Integer.parseInt(matcher.group(1)); handler.setCardCount(c); } catch
		 * (NumberFormatException e) { e.printStackTrace(); } } else if
		 * (cardItemPattern.matcher(line).find()) { parseRecord(line, handler); cards =
		 * true; continue; } else if (lastPagePattern.matcher(line).find()) { lastpage =
		 * true; } } if (cards == false) throw new RuntimeException("No results");
		 * return lastpage;
		 */
	}
	...
	public static void main(String[] args) throws MalformedURLException, IOException {
		...
		new ParseGathererSearchChecklist()
				.loadSingleUrl(GatherHelper.getSearchQuery("checklist&sort=cn+", "Magic 2013", false), handler);
