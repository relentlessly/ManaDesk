...
public abstract class ParserHtmlHelper {
	...
	public static class OutputHandler implements ILoadCardHander {
		...
		protected void printCard(MagicCard card) {
	...
	public static class StashLoadHandler implements ILoadCardHander {
		...
		private ArrayList<MagicCard> secondary = new ArrayList<>();
	...
	public static URL createImageURL(int cardId) {
		try {
			return new URL(
					GatherHelper.GATHERER_URL_BASE + "Handlers/Image.ashx?multiverseid=" + cardId + "&type=card");
	...
	public static URL createManaImageURL(String symbol) {
		...
		try {
			return new URL(
					"http://gatherer.wizards.com/Handlers/Image.ashx?size=small&name=" + manaName + "&type=symbol");
		} catch (MalformedURLException e) {
			return null;
		}
	}
