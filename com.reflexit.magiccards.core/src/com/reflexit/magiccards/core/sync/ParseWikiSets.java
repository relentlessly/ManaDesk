...
public class ParseWikiSets extends AbstractParseHtmlPage {
	...
	protected String getUrl() {
		...
		/*
		 * try { String setE = URLEncoder.encode("List_of_Magic:_The_Gathering_sets",
		 * "UTF-8"); return SET_QUERY_URL_BASE + setE; } catch
			...
			if (line.contains("<th")) {
				...
				if (headers == null) {
					...
					for (int i = 0; i < headers.size(); i++) {
						...
						if (headerName.equalsIgnoreCase("Release date")) {
							releaseIndex = i;
						}
						if (headerName.equalsIgnoreCase("Expansion code") || headerName.equalsIgnoreCase("Set code")
								|| headerName.equalsIgnoreCase("Expansioncode")) {
							setCode = i;
						}
					}
				}
