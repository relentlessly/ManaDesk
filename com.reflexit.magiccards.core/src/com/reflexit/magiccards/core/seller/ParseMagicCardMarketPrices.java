...
public class ParseMagicCardMarketPrices extends AbstractPriceProvider {
	private final String setURL = "https://www.cardmarket.com/en/Magic/AdvancedSearch";
	private final String baseURL = "https://www.cardmarket.com/en/Magic/AdvancedSearch?doSubmit=1&expansion[]=SET&site=PAGE";
	...
	public Iterable<IMagicCard> updatePrices(final Iterable<IMagicCard> iterable,
		...
		for (IMagicCard magicCard : iterable) {
			if (offlineSets.containsKey(magicCard.getSet())) {
				...
				offlineSets.put(magicCard.getSet(), new ArrayList<>());
			}
		...
		for (String element : offlineSets.keySet()) {
			actions.add(new Action(monitor, element, onlineSets, offlineSets));
		}
		...
		for (Entry<Action, Map<String, Float>> entry : result.entrySet()) {
		}
	...
	private class Action implements Callable<Map<String, Float>> {
		...
		private void parseSingleCard(String setId, IMagicCard card) throws IOException {
			String cardName = changeNameDBToHTML(card.getEnglishName(), card.getFlipId() != null);
	...
	public static void main(String[] args) throws IOException {
		...
		Map<String, String> onlineSets = item.getOnlineSets();
		System.err.println("Sets: " + onlineSets);
	}
}
