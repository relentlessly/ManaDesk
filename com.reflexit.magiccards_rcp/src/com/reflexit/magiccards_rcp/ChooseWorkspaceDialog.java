...
public class ChooseWorkspaceDialog extends TitleAreaDialog {
	 ...
	 *            indicates whether the dialog should be centered on the monitor or according to
	 *            it's parent if there is one
	 */
	...
	/**
	 * Show the dialog to the user (if needed). When this method finishes, #getSelection will return
	 * the workspace that should be used (whether it was just selected by the user or some previous
	 * default has been used. The parameter can be used to override the users preference. For
	 * example, this is important in cases where the default selection is already in use and the
	 * user is forced to choose a different one.
	 * 
	 * @param force
	 *            true if the dialog should be opened regardless of the value of the show dialog
	 *            checkbox
	 */
	public void prompt(boolean force) {
		Activator.log("need prompt for workspace: " + launchData.getShowDialog());
	...
	protected Control createDialogArea(Composite parent) {
		...
		setTitle(MAWorkbenchMessages.ChooseWorkspaceDialog_dialogTitle);
		setMessage(NLS.bind(MAWorkbenchMessages.ChooseWorkspaceDialog_dialogMessage, productName));
		...
		if (!gcj && vmName != null && vmName.indexOf("libgcj") != -1) { //$NON-NLS-1$
			composite.getDisplay().asyncExec(new Runnable() {
				public void run() {
					...
					setMessage(MAWorkbenchMessages.UnsupportedVM_message, IMessageProvider.WARNING);
				}
			});
		}
	...
	public static String getWindowTitle() {
		...
		if (productName == null) {
			productName = MAWorkbenchMessages.ChooseWorkspaceDialog_defaultProductName;
		}
		return productName;
	}
	 ...
	 * <p>
	 * The default implementation of this framework method sets the shell's image and gives it a
	 * grid layout. Subclasses may extend or reimplement.
	 * </p>
	 * 
	 * @param shell
	 *            the shell
	...
	protected void configureShell(Shell shell) {
		...
		shell.setText(MAWorkbenchMessages.ChooseWorkspaceDialog_dialogName);
	}
	 ...
	 * <p>
	 * The <code>Dialog</code> implementation of this framework method sets this dialog's return
	 * code to <code>Window.CANCEL</code> and closes the dialog. Subclasses may override if desired.
	 * </p>
	 */
	@Override
	...
	/**
	 * The main area of the dialog is just a row with the current selection information and a
	 * drop-down of the most recently used workspaces.
	 */
	private void createWorkspaceBrowseRow(Composite parent) {
		...
		label.setText(MAWorkbenchMessages.ChooseWorkspaceDialog_workspaceEntryLabel);
		...
		browseButton.setText(MAWorkbenchMessages.ChooseWorkspaceDialog_browseLabel);
		...
		browseButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				dialog.setText(MAWorkbenchMessages.ChooseWorkspaceDialog_directoryBrowserTitle);
				dialog.setMessage(MAWorkbenchMessages.ChooseWorkspaceDialog_directoryBrowserMessage);
	...
	/**
	 * Return a string containing the path that is closest to the current selection in the text
	 * widget. This starts with the current value and works toward the root until there is a
	 * directory for which File.exists returns true. Return the current working dir if the text box
	...
	private void createShowDialogButton(Composite parent) {
		...
		button.setText(MAWorkbenchMessages.ChooseWorkspaceDialog_useDefaultMessage);
